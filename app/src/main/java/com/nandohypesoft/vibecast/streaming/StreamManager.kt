...
class StreamManager(private val context: Context) {
    ...
    fun startStream(config: StreamConfig) {
        ...
        val command = "-f s16le -ar 44100 -ac 1 -i $pipePath -c:a libmp3lame -b:a ${config.bitrate}k -legacy_icecast 1 -content_type audio/mpeg -ice_name \"VibeCast Studio\" -tune zerolatency -preset ultrafast -fflags nobuffer -flush_packets 1 -f mp3 $url"
    ...
    fun updateMetadata(config: com.nandohypesoft.vibecast.data.StreamConfig, metadata: String) {
